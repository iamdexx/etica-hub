 ...
 * live ETX balance off chain 61803 before recording a moderation vote.
 *
...
import {
  MAX_SIG_AGE_MS,
  MIN_VOTE_BALANCE_ETX,
  voteMessage,
  type ModTarget,
} from './moderation-store';

const ETX_DECIMALS = 18n;
 ...
 * current ETX balance off chain 61803. Returns the canonical checksum
...
export async function verifyVotePayload(input: VerifyInput): Promise<VerifyResult> {
  ...
  const etx = DEPLOYMENTS[61803]?.etx as Address | undefined;
  if (!etx) {
    return { ok: false, status: 503, error: 'ETX address is not configured.' };
  }
  let balance: bigint;
  try {
    ...
    return {
      ok: false,
      status: 502,
      error: `Failed to read ETX balance: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  if (balance < MIN_VOTE_BALANCE_ETX) {
    return {
      ok: false,
      status: 403,
      error: `Wallet holds ${(balance / 10n ** ETX_DECIMALS).toString()} ETX; moderation requires ≥ ${(MIN_VOTE_BALANCE_ETX / 10n ** ETX_DECIMALS).toString()} ETX.`,
    };
  }
